...
void balance(Node *&x) {
    ...
    if(x == NULL || (x->left == NULL && x->right == NULL)) {
        ...
        // height of null left node is -1, get height at right
        leftHeight = -1;
        rightHeight = x->right->height;

    } else if (x->right == NULL) {
        // height of null right node is -1, get height at left
        rightHeight = -1;
        leftHeight = x->left->height;

    } else {
        // no null nodes; get height of left and right
        leftHeight = x->left->height;
        rightHeight = x->right->height;
    }

    // calculate height of left node relative to height of right node
...
int traversalValues[] = {70, 0, 0,
                         ...
                         70, 0, 2, 60, 1, 1, 50, 3, 0, 40, 0, 3, 30, 1, 2, 27, 0, 3, 25, 2, 1, 20, 0, 3, 10, 1, 2, 5, 0,
                         3};
